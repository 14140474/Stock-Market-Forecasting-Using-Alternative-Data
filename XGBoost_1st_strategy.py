...
for company_name, (sheet_name, ticker) in sheet_names_map.items():
    print(f" Processing Models for: {company_name}")
    ...
    # features with non-lagged sentiment
    ...
    models_to_run = {"Baseline": base_features, "Attention": attention_features,
        "Sentiment": sentiment_features, "Hybrid": hybrid_features}

    for model_name, features in models_to_run.items():
        print(f"Training {model_name} model for {company_name}")
        ...
        model = xgb.XGBClassifier(objective='binary:logistic', n_estimators=1000, learning_rate=0.05,
...
if all_model_results:
    ...
    final_filename = 'final_summary_and_stats_xgboost.csv'

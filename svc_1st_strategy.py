...
# Feature Engineering 
print("\n--- 4. Engineering Features for Modeling ---")
df_model_ready = df_analysis.copy()

# Apply Log Transformation to specified variables to reduce skewness 
print("Applying log transformation to skewed variables...")
vars_to_log_transform = (
    sentiment_count_variables +['inflation_rescaled', 'stocks_rescaled', 
                                'companyname_rescaled', 'companystock_rescaled'])
for col in vars_to_log_transform:
    if col in df_model_ready.columns:
        # Using np.log1p which calculates log(1 + x) to handle zeros
        df_model_ready[col] = np.log1p(df_model_ready[col])
...
print("\n" + "="*50)
print("--- 5. Training All Models for Each Company ---")
print("="*50 + "\n")
...
for company_name, (sheet_name, ticker) in sheet_names_map.items():
    ...
    for model_name, features in models_to_run.items():
        ...
        model = SVC(
            kernel='rbf', # Radial Basis Function is a powerful and common choice
            random_state=42)
...
if all_model_results:
    ...
    final_filename = 'final_summary_and_stats_svc_log.csv'
